...
- `Fluidsynth`: the default portamento setting matches Fluidsynth's portamento implementation. One should add a CC65 with value 127 (enable Portamento) and a CC67 with value 127 (enable Legato to make it monophonic if needed) at the beginning of the sequence unless these options are already set
